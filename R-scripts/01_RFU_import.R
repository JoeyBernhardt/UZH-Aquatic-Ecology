...
RFU_files <- c(list.files("data-raw/absorbance", full.names = TRUE))
RFU_files <- RFU_files[grepl(".xls", RFU_files)]
...
all_plates <- map_df(RFU_files, read_excel, range = "B25:N33", .id = "file_name") %>% 
	rename(row = ...1) %>% 
	mutate(file_name = str_replace(file_name, " ", ""))
...
write_csv(all_rfus3, "data-processed/absorbances.csv")

library(broom)
growth_rates <- all_rfus3 %>% 
	group_by(well_plate, acclimation_conditions, experiment_conditions, replicate, population) %>% 
	do(tidy(lm(log(absorbance) ~ days, data = .), conf.int = TRUE)) %>% 
	filter(term != "(Intercept)") %>% 
	group_by(population, acclimation_conditions, experiment_conditions) %>% 
	summarise_each(funs(mean), estimate) 

write_csv(growth_rates, "data-processed/absorbance-growth-rates.csv")
